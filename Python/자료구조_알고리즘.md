...
#### 유니코드
- 전 세계의 모든 문자를 컴퓨터에서 일관되게 다룰 수 있도록 설계된 산업표준 코드
- 유니코드는 1바이트로 표현이 가능한 아스키코드도 2바이트 이상의 공간을 사용하기 때문에 UTF-8(가변 길이 문자 인코딩 방식)인코딩 방식을 사용
- 유니코드는 UTF(가변 길이 문자 인코딩 방식)뿐만 아니라 UCS(고정 바이트 인코딩 방식)도 사용
- 파이썬에서는 슬라이싱과 인덱스 접근을 위해 내부적으로는 UCS사용한다고 함(파이썬 알고리즘 인터뷰 175p)
#### UTF-8
- UTF-8(Universal Coded Character Set + Transformation Format – 8-bit)은 유니코드를 위한 가변 길이 문자 인코딩 방식
- 유니코드 한 문자를 표현하기 위해 1바이트~4바이트를 사용
- 유니코드는 문자 비트를 최대 4바이트로 나누어서 표현
- ASCII 문자들은 UTF-8에서 1바이트로 표현
- 2바이트 부터는 첫 바이트에 2 = 110, 3 = 1110, 4 = 11110 로 시작하고 나머지 바이트들은 10으로 시작
#### 선형 자료구조
...
##### 동적배열
- 크기를 지정할 필요 없음
- 미리 초기값을 작게 배열을 생성 후, 데이터 추가로 꽉 차면 배열 크기를 늘림
- 대부분의 프로그래밍 언어는 동적배열 지원 (ex. 파이썬: 리스트, 자바: ArrayList, C++: std::vector)
- 대부분의 동적 프로그래밍 언어들은 정적 배열 지원x(ex. 파이썬)
- 조회는 O(1)
- 배열을 늘리는 과정에서 최악의 경우O(n)발생하고, 분할 상환 분석에 따른 입력시간은 여전히 O(1)
- 분할 상환 분석에 따른 시간복잡도를 설명하는 대표적인 자료형
##### 재할당 비율
- 재할당 비율을 그로스 팩터, 즉 성장 인자라고 함
- 파이썬 0, 4, 8, 16, 25, 35..... 초반에 2배(더블링), 전체적으로는 약 1.125배
- 자바 ```ArrayList```는 1.5배, C++ std:vector나 루비언어는 2배(더블링)
###### etc 
- 브루트 포스: 모든 조합을 계산하는 무차별 대입 방식
##### 투 포인터
- 시작점과 끝점, 왼쪽 포인터와 오른쪽 포인터 두 지점을 기준으로 하는 문제 풀이 전략
- 주로 정렬된 배열에서 사용

##### 최대값과 최소값
- 최대값에는 가장낮은 값, 최소값에는 가장높은 값을 지정해야 원활히 교체가능
- 최대값에는 -sys.maxsize, 최소값에는 sys.maxsize 지정 (import sys 필수)
- 양의 무한대 = float('inf') 음의 무한대 = float('-inf')
- 문제의 조건에 맞게 최대값 최소값 설정
- ```python
  # Example
  import sys
  min = -sys.maxsize
  max = sys.maxsize
  print(f'min: {min}, max: {max}')    # min: -9223372036854775807, max: 9223372036854775807

  # Example
  import sys
  max = float('-inf')
  min = float('inf')
  print(max, min)  # -inf inf
  if min>sys.maxsize:
      print('양의 무한대')    # 양의 무한대
  ```
